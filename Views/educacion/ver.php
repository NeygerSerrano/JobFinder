<?php 
$title = "Educaciones registradas"; 
require './Views/layouts/header.php'; 
?>

<div class="max-w-6xl mx-auto p-6">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">🎓 Educaciones Registradas</h2>
                ...
                <tbody class="divide-y divide-gray-200">
                    <?php foreach ($educaciones as $edu): ?>
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2"><?= htmlspecialchars($edu->getEntidad()) ?></td>
                            <td class="px-4 py-2"><?= htmlspecialchars($edu->getDescripcion()) ?></td>
                            <td class="px-4 py-2"><?= htmlspecialchars($edu->getNro_doc_persona()) ?></td>
                            <td class="px-4 py-2 space-x-2">
                                <a href="index.php?controller=educacion&action=editar&id_estudio=<?= urlencode($edu->getId_estudio()) ?>"
                                   class="text-blue-600 hover:text-blue-800 font-medium">📝 Editar</a>
                                |
                                <a href="index.php?controller=educacion&action=eliminar&id_estudio=<?= urlencode($edu->getId_estudio()) ?>&nro_doc_persona=<?= urlencode($edu->getNro_doc_persona()) ?>"
                                   onclick="return confirm('¿Deseas eliminar esta educación?');"
    ...
    <?php else: ?>
        <p class="text-gray-600 mb-4">No hay educaciones registradas.</p>
    <?php endif; ?>

    <!-- Acciones -->
    <div class="mt-6 space-y-3">
        <a href="index.php?controller=educacion&action=crear"
        ...
        <br>
        <button onclick="window.location.href='index.php?controller=educacion&action=index'" 
